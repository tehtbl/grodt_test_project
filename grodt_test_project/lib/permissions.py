...
from users.models import MyUser, ObjectAccess
...
def get_account_roles(user, account=None):
    ...
    result = [core_constants.ROLE_SIMPLEUSERS]
    ...
    if user.is_superuser:
        result += [
            core_constants.ROLE_NORMALADMINS,
            core_constants.ROLE_SUPERADMINS
        ]
...
def grant_access_to_object(user, obj, is_owner=False):
    ...
    * He is going to administrate the object (but he is not the owner)
    ...
    # entry, created = ObjectAccess.objects.get_or_create(user=user, content_type=ct, object_id=obj.uuidpk)
    entry, created = ObjectAccess.objects.get_or_create(user=user, content_type=ct, object_id=obj.pk)
    entry.is_owner = is_owner
    entry.save()
    if not created or not is_owner:
        return
    for su in MyUser.objects.filter(is_superuser=True):
        if su == user:
            continue
        ObjectAccess.objects.get_or_create(
            user=su, content_type=ct, object_id=obj.pk
        )
...
def ungrant_access_to_object(obj, user=None):
    ...
    if user is not None:
        ...
        try:
            ObjectAccess.objects.get(
                content_type=ct, object_id=obj.pk, is_owner=True
            )
        except ObjectAccess.DoesNotExist:
            grant_access_to_object(
                MyUser.objects.filter(is_superuser=True)[0], obj, True
            )
    else:
...
def ungrant_access_to_objects(objects):
    ...
    for obj in objects:
        ct = ContentType.objects.get_for_model(obj)
        ObjectAccess.objects.filter(content_type=ct, object_id=obj.pk).delete()
...
def add_permissions_to_group(group, permissions):
    ...
    for appname, modelname, permname in permissions:
        # logging.critical("app {} model {} perm {}".format(appname, modelname, permname))
        ...
        group.permissions.add(Permission.objects.get(content_type=ct, codename=permname))
