...
def get_model_name(s):
    ...
    first = val[0] if len(val) > 0 else ''
    remaining = val[1:] if len(val) > 1 else ''
...
def render_tags(tags):
    t = Template(
        """{% for tag in tags %}<span class="label label-{% if tag.color %}{{ tag.color }}{% else %}default{% endif %}"><a href="#" class="filter" name="{{ tag.name }}">{{ tag.label }}</a></span>{% endfor %}""")  # NOQA:E501
    return t.render(Context({"tags": tags}))
...
def extra_static_content(caller, st_type, user):
    ...
    return tpl.render(
        template.Context({"static_content": ""})
    )
